...
func GenerateKey2() (vrf.PrivateKey, vrf.PublicKey, string) {
	...
	return &PrivateKey{PrivateKey: key}, &PublicKey{PublicKey: &key.PublicKey}, pubkeyStr
}

func generatePrivateKey(c elliptic.Curve) ([]byte, string, *ecdsa.PrivateKey) {
	priv, _ := ecdsa.GenerateKey(c, rand.Reader)
	privateKeyInByte := priv.D.Bytes()
	privateKeyInString := hex.EncodeToString(privateKeyInByte)
	return privateKeyInByte, privateKeyInString, priv
}

func getPrivateKey(priv *ecdsa.PrivateKey) string {
	privateKeyInByte := priv.D.Bytes()
	privateKeyInString := hex.EncodeToString(privateKeyInByte)
	return privateKeyInString
}

func RestoreKeyPair(privateKey string) (vrf.PrivateKey, []byte, error) {
	c := curve
	privateKeydecodeToByte, err := hex.DecodeString(privateKey)
	if err != nil {
		return nil, nil, err
	}
	k := hashToInt(privateKeydecodeToByte, c)
	priv := new(ecdsa.PrivateKey)
	priv.PublicKey.Curve = c
	priv.D = k
	priv.PublicKey.X, priv.PublicKey.Y = c.ScalarBaseMult(k.Bytes())
	pubKey := append(priv.PublicKey.X.Bytes(), priv.PublicKey.Y.Bytes()...)
	return &PrivateKey{PrivateKey: priv}, pubKey, nil
}
...
// GenerateKey generates a fresh keypair for this VRF
func GenerateKeyPair() (vrf.PrivateKey, []byte, []byte, error) {
	key, err := ecdsa.GenerateKey(curve, rand.Reader)
	if err != nil {
	...
	return &PrivateKey{PrivateKey: key}, key.D.Bytes(), pubKey, nil
}
...
func String2Pubkey(publicKeyStr string) vrf.PublicKey {
	...
	return &PublicKey{PublicKey: rawPubKey}
}
...
func H1(m []byte) (x, y *big.Int) {
	h := sha512.New()
	var i uint32
	byteLen := (curveParams.BitSize + 7) >> 3
	for x == nil && i < 100 {
...
func H2(m []byte) *big.Int {
	...
	for i := uint32(0); ; i++ {
		...
		if k.Cmp(new(big.Int).Sub(curveParams.N, one)) == -1 {
			return k.Add(k, one)
		}
	}
}
...
func (k PrivateKey) Evaluate(m []byte) (index [32]byte, proof []byte) {
	...
	// VRF_k(m) = [k]H
	sHx, sHy := curveParams.ScalarMult(Hx, Hy, k.D.Bytes())
	vrf := elliptic.Marshal(curve, sHx, sHy) // 65 bytes.

	// G is the base point
	// s = H2(G, H, [k]G, VRF, [r]G, [r]H)
	rGx, rGy := curveParams.ScalarBaseMult(r)
	rHx, rHy := curveParams.ScalarMult(Hx, Hy, r)
	var b bytes.Buffer
	b.Write(elliptic.Marshal(curve, curveParams.Gx, curveParams.Gy))
...
func (pk *PublicKey) ProofToHash(m, proof []byte) (index [32]byte, err error) {
	...
	// [t]G + [s]([k]G) = [t+ks]G
	tGx, tGy := curveParams.ScalarBaseMult(t)
	ksGx, ksGy := curveParams.ScalarMult(pk.X, pk.Y, s)
	tksGx, tksGy := curveParams.Add(tGx, tGy, ksGx, ksGy)

	// H = H1(m)
	// [t]H + [s]VRF = [t+ks]H
	Hx, Hy := H1(m)
	tHx, tHy := curveParams.ScalarMult(Hx, Hy, t)
	sHx, sHy := curveParams.ScalarMult(uHx, uHy, s)
	tksHx, tksHy := curveParams.Add(tHx, tHy, sHx, sHy)
...
func (pk *PublicKey) Verify(seed, proof []byte, index [32]byte) (bool, error) {
	r, err := pk.ProofToHash(seed, proof)
	if err != nil {
		return false, err
	}
	if r == index {
...
//func Verify(publicKey []byte, seed, proof []byte, index [32] byte) (bool, error) {
//
//	publicKeyVrf := String2Pubkey(publicKey)
//
//	return publicKeyVrf.Verify(seed, proof, index)
//
//}

func Verify(publicKey string, seed, proof []byte, index [32]byte) (bool, error) {
	publicKeyVrf := String2Pubkey(publicKey)
	return publicKeyVrf.Verify(seed, proof, index)
}
