...
func (c *jsonCodec) ParseRequestArguments(argTypes []reflect.Type, parameters interface{}) ([]reflect.Value, Error) {
	if args, ok := parameters.(json.RawMessage); !ok {
...
// CreateNotification will create a JSON-RPC notification with the given subscription id and event as config.
