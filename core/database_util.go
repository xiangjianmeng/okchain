...
func GetBlock(db DatabaseReader, hash common.Hash, number uint64) *types.Block {
	...
	block.DposContext = getDposContextTrie(db.(database.Database), header)
	return block
}

func getDposContextTrie(db database.Database, header *types.Header) *types.DposContext {
...
// func WriteTd(db database.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteChainConfig(db database.Putter, hash common.Hash, cfg *config.ChainConfig) error {
...
func WriteHeaderHash(db database.Putter, hash common.Hash) error {
	if err := db.Put(headHeaderKey, hash.Bytes()); err != nil {
		//@wei.ni log input
		return err
	}
