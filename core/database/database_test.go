...
func newTestLDB() (*LDBDatabase, func()) {
	dirname, err := ioutil.TempDir(os.TempDir(), "okcdb_test_")
	if err != nil {
...
var test_values = []string{"bbb", "a", "1251", "\x00123\x00"}
...
func testPutGet(db Database, t *testing.T) {
	...
	for _, v := range test_values {
		...
		if err == nil && len(value) > 0 {
			t.Fatalf("got deleted value %q", value)
		}
	}
}
...
func newTestRDB() (*RDBDatabase, func()) {
	dirname, err := ioutil.TempDir(os.TempDir(), "okcdb_test_")
	if err != nil {
		panic("failed to create test file: " + err.Error())
	}
	db, err := NewRDBDatabase(dirname, 0, 0)
	if err != nil {
		panic("failed to create test database: " + err.Error())
	}
...
func TestRocksDB_allinone(t *testing.T) {
	path := "/tmp/rocks_tmp/"
	db, err := NewRDBDatabase(path, 16, 1024)
	assert.True(t, db != nil && err == nil)

	key := []byte("a")
	value := []byte("b")
	db.Put(key, value)
	gotValue, _ := db.Get(key)
	assert.True(t, bytes.Compare(gotValue, value) == 0)
