...
func SetupGenesisBlock(db database.Database, genesis *Genesis) (*config.ChainConfig, common.Hash, error) {
	...
	if (stored == common.Hash{}) {
		...
		fmt.Println("Read genesis block from db")
	}
	...
	if err != nil {
		if err == ErrChainConfigNotFound {
			...
			log.Warning("Found genesis block without chain config")
...
func (g *Genesis) Commit(db database.Database) (*types.Block, error) {
	block := g.ToBlock(db)
	// add dposcontext
	if _, err := block.DposContext.Commit(nil); err != nil {
		return nil, err
	}
	...
	gConfig := g.Config
	if gConfig == nil {
		gConfig = config.AllEthashProtocolChanges
	}
	return block, WriteChainConfig(db, block.Hash(), gConfig)
}
...
func initGenesisDposContext(g *Genesis, db database.Database) *types.DposContext {
