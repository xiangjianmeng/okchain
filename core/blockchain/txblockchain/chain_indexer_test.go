...
func testChainIndexer(t *testing.T, count int) {
	...
	for i := 0; i < count; i++ {
		...
		backends[i].indexer = NewChainIndexer(db, okcdb.NewTable(db, string([]byte{byte(i)})), backends[i], sectionSize, confirmsReq, 0, fmt.Sprintf("indexer-%d", i))
	...
	inject := func(number uint64) {
		...
		if number > 0 {
			header.PreviousBlockHash = rawdb.ReadCanonicalHash(db, number-1).Bytes()
		}
...
func (b *testChainIndexBackend) Process(header *protos.TxBlockHeader) {
	b.headerCnt++
	...
	case b.processCh <- header.GetBlockNumber():
	}
}
...
//
//func TestChainIndexer_FullLifeCycle(t *testing.T) {
//
//	// 1. Initialize & enter updateLoop and eventLoop.
//	txBC := getDefaultTxBlockChain(false, "_chain_index")
//
//	chainIdxer := txblockchain.NewBloomIndexer(txBC.GetDatabase(), testSectionSize)
//	chainIdxer.Start(txBC.(*TxBlockChain))
//
//	// 2. Insert $testSectionSize txBlock into TxBlockchain.
//	//    A -- 50000000000 --> B
//	block1 := getDefaultTxBlock(txBC, 50000000000)
//	err := txBC.InsertBlock(block1)
//	assert.True(t, err == nil)
//
//	for i := 0; i < testSectionSize*2+1; i++ {
//		b := getDefaultSmartContractTxBlock(txBC, 10000, nil, nil)
//		txBC.InsertBlock(b)
//	}
//	time.Sleep(time.Second * 10)
//
//	// 3. Check post-process BloomIndex of a block section.
//	proccessedSec, processedBlockNO, headHash := chainIdxer.Sections()
//	//chainIdxer.log.Info(proccessedSec, processedBlockNO, headHash.Hex())
//	fmt.Println(proccessedSec)
//	assert.True(t, proccessedSec > 0)
//
//	chainIdxer.loadValidSections()
//	storedSections := chainIdxer.storedSections
//	assert.True(t, proccessedSec == storedSections)
//
//	// 4. Close ChainIndexer
//	chainIdxer.Close()
