...
// NewTrieWithPrefix NewTrieWithPrefix
func NewTrieWithPrefix(root common.Hash, prefix []byte, db database.Database) (*Trie, error) {
	trie, err := New(root, NewDatabase(db))
	//h := trie.Hash()
	//fmt.Printf("root: %s, prefix: %s, trie.Hash: %s\n", hex.EncodeToString(root[:]), string(prefix), hex.EncodeToString(h[:]))
...
func (t *Trie) RootHash() common.Hash { return t.Hash() }
