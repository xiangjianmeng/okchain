...
func (bc *testBlockChain) CurrentBlock() *types.TxBlock {
...
func (bc *testBlockChain) GetBlock(hash common.Hash, number uint64) *types.TxBlock {
...
func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *ecdsa.PrivateKey) *types.Transaction {
	// tx, _ := types.SignTx(types.NewTransaction(nonce, nil, common.Address{}, 100, gaslimit, gasprice, nil), types.P256Signer{Prv: key})
	// return tx
	return nil
}
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		memdb, _ := database.NewMemDatabase()
		c.statedb, _ = state.New(common.Hash{}, state.NewDatabase(memdb))
...
func TestInvalidTransactions(t *testing.T) {
	...
	balance := new(big.Int).Add(tx.Value(), new(big.Int).Mul(new(big.Int).SetUint64(tx.Gas()), new(big.Int).SetUint64(tx.GasPrice)))
...
// 	tx, _ := types.SignTx(types.NewTransaction(0, nil, common.Address{}, big.NewInt(-1), 100, big.NewInt(1), nil), types.P256Signer{key})
// 	from, _ := deriveSender(tx)
// 	pool.currentState.AddBalance(from, big.NewInt(1))
...
func TestTransactionChainFork(t *testing.T) {
	...
	// addr := types.P256Signer{}.PubKeyToAddress(&key.PublicKey)
	// resetState := func() {
	// 	memdb, _ := database.NewMemDatabase()
	// 	statedb, _ := state.New(common.Hash{}, state.NewDatabase(memdb))
...
func TestTransactionDoubleNonce(t *testing.T) {
	...
	// signer := types.P256Signer{Prv: key}

	// // addr := crypto.PubkeyToAddress(key.PublicKey)
	// addr := signer.PubKeyToAddress(&key.PublicKey)
	// resetState := func() {
	// 	memdb, _ := database.NewMemDatabase()
	// 	statedb, _ := state.New(common.Hash{}, state.NewDatabase(memdb))
	// 	statedb.AddBalance(addr, big.NewInt(100000000000000))
	...
	// // pubKey := signer.PubKeyToBytes(&key.PublicKey)
	// tx1, _ := types.SignTx(types.NewTransaction(0, nil, common.Address{}, 100, 100000, big.NewInt(1), nil), signer)
	// tx2, _ := types.SignTx(types.NewTransaction(0, nil, common.Address{}, 100, 1000000, big.NewInt(2), nil), signer)
	// tx3, _ := types.SignTx(types.NewTransaction(0, nil, common.Address{}, 100, 1000000, big.NewInt(1), nil), signer)
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	// pool := NewTxPool(config, config.TestChainConfig, blockchain)
	// defer pool.Stop()
...
func testTransactionQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	// local, _ := ecdsa.GenerateKey(elliptic.P256(), crand.Reader)
	// remote, _ := ecdsa.GenerateKey(elliptic.P256(), crand.Reader)
	...
	// pool.currentState.AddBalance(types.P256Signer{}.PubKeyToAddress(&local.PublicKey), big.NewInt(1000000000))
	// pool.currentState.AddBalance(types.P256Signer{}.PubKeyToAddress(&remote.PublicKey), big.NewInt(1000000000))
...
func testTransactionJournaling(t *testing.T, nolocals bool) {
	...
	// 	t.Fatalf("pool internal state corrupted: %v", err)
	// }
	// fmt.Println("before stop:", pending, queued, pool.all.Count(), len(pool.Locals()), pool.journal)
	...
	// pending, queued = pool.Stats()
	// fmt.Println("after restart:", pending, queued, pool.all.Count(), len(pool.Locals()), pool.journal)
	// if queued != 0 {
	// 	t.Fatalf("queued transactions mismatched: have %d, want %d", queued, 0)
	// }
	// if nolocals {
