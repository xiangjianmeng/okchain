...
// 跟踪IP，确保至多有N个属于同一个网络范围。2018-04-18 郑超
type DistinctNetSet struct {
	Subnet uint // 常量24. number of common prefix bits. 共同前缀位的个数？也就是同一个网段的意思
	Limit  uint // 常量table 10 / bucket 2. maximum number of IPs in each subnet. 一个子网里最多有多少个IP

	members map[string]uint // 比如是IPv4，则 map [52, 类192, 类168 , 类159]  数量
	buf     net.IP
}

// Add adds an IP address to the set. It returns false (and doesn't add the IP) if the
// number of existing IPs in the defined range exceeds the limit.
// 添加IP到集合。如果在定义范围内已存在的IP数量超过限制，它返回false（并且不添加这个IP）2018-04-18 郑超
...
// key 将地址密钥编码为临时缓冲区。2018-04-18 郑超
