...
type GossipChannel interface {
	...
	// UpdateDsLedgerHeight updates the ledger height the peer
	// publishes to other peers in the channel
	UpdateDsLedgerHeight(height uint64)

	// UpdateTxLedgerHeight updates the ledger height the peer
	// publishes to other peers in the channel
	UpdateTxLedgerHeight(height uint64)
...
type gossipChannel struct {
	...
	txMsgStore                msgstore.MessageStore
	...
	dsLedgerHeight            uint64
	txLedgerHeight            uint64
	incTime                   uint64
	leftChannel               int32
}
...
func NewGossipChannel(pkiID common.PKIidType, org api.OrgIdentityType, mcs api.MessageCryptoService,
	...
	gc.txMsgStore = msgstore.NewMessageStoreExpirable(comparator, func(m interface{}) {},
		gc.GetConf().BlockExpirationInterval, nil, nil, func(m interface{}) {})
...
func (gc *gossipChannel) LeaveChannel() {
	...
	var dsheight, txheight uint64
	if prevMsg := gc.stateInfoMsg; prevMsg != nil {
		chaincodes = prevMsg.GetStateInfo().Properties.Chaincodes
		dsheight = prevMsg.GetStateInfo().Properties.DsLedgerHeight
		txheight = prevMsg.GetStateInfo().Properties.TxLedgerHeight
	}
	gc.updateProperties(dsheight, txheight, chaincodes, true)
}
...
func (gc *gossipChannel) AddToMsgStore(msg *proto.SignedGossipMessage) {
	...
	if msg.IsTransactionMsg() {
		gc.txMsgStore.Add(msg)
	}
}
...
func (gc *gossipChannel) HandleMessage(msg proto.ReceivedMessage) {
	...
	if m.IsDataMsg() || m.IsStateInfoMsg() || m.IsTransactionMsg() {
		added := false
		...
		} else if m.IsTransactionMsg() {
			added = gc.txMsgStore.Add(msg.GetGossipMessage())
		}
...
// publishes to other peers in the channel
func (gc *gossipChannel) UpdateTxLedgerHeight(height uint64) {
	gc.Lock()
	defer gc.Unlock()

	var chaincodes []*proto.Chaincode
	var leftChannel bool
	var dsLedgerHeight uint64 = 1
	if prevMsg := gc.stateInfoMsg; prevMsg != nil {
		leftChannel = prevMsg.GetStateInfo().Properties.LeftChannel
		chaincodes = prevMsg.GetStateInfo().Properties.Chaincodes
		dsLedgerHeight = prevMsg.GetStateInfo().Properties.DsLedgerHeight
	}
	gc.updateProperties(dsLedgerHeight, height, chaincodes, leftChannel)
}

// UpdateLedgerHeight updates the ledger height the peer
// publishes to other peers in the channel
func (gc *gossipChannel) UpdateDsLedgerHeight(height uint64) {
	gc.Lock()
	defer gc.Unlock()

	var chaincodes []*proto.Chaincode
	var leftChannel bool
	var txLedgerHeight uint64 = 1
	if prevMsg := gc.stateInfoMsg; prevMsg != nil {
		leftChannel = prevMsg.GetStateInfo().Properties.LeftChannel
		chaincodes = prevMsg.GetStateInfo().Properties.Chaincodes
		txLedgerHeight = prevMsg.GetStateInfo().Properties.TxLedgerHeight
	}
	gc.updateProperties(height, txLedgerHeight, chaincodes, leftChannel)
}
...
func (gc *gossipChannel) UpdateChaincodes(chaincodes []*proto.Chaincode) {
	...
	var dsLedgerHeight, txLedgerHeight uint64 = 1, 1
	var leftChannel bool
	if prevMsg := gc.stateInfoMsg; prevMsg != nil {
		dsLedgerHeight = prevMsg.GetStateInfo().Properties.DsLedgerHeight
		txLedgerHeight = prevMsg.GetStateInfo().Properties.TxLedgerHeight
	...
	gc.updateProperties(dsLedgerHeight, txLedgerHeight, chaincodes, leftChannel)
}
...
func (gc *gossipChannel) updateStateInfo(msg *proto.SignedGossipMessage) {
	gc.stateInfoMsgStore.Add(msg)
	gc.dsLedgerHeight = msg.GetStateInfo().Properties.DsLedgerHeight
	gc.txLedgerHeight = msg.GetStateInfo().Properties.TxLedgerHeight
	gc.stateInfoMsg = msg
	atomic.StoreInt32(&gc.shouldGossipStateInfo, int32(1))
}

func (gc *gossipChannel) updateProperties(dsLedgerHeight, txLedgerHeight uint64, chaincodes []*proto.Chaincode, leftChannel bool) {
	stateInfMsg := &proto.StateInfo{
		...
		Properties: &proto.Properties{
			...
			DsLedgerHeight: dsLedgerHeight,
			TxLedgerHeight: txLedgerHeight,
