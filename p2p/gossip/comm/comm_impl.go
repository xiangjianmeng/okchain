...
const (
	...
	defRecvBuffSize = 3000
	defSendBuffSize = 3000
)
...
func (c *commImpl) SetDialOpts(opts ...grpc.DialOption) {
	if len(opts) == 0 {
		c.logger.Debugf("Given an empty set of grpc.DialOption, aborting")
		return
	}
	c.opts = opts
}
	...
	if stream, err = cl.GossipStream(ctx); err == nil {
		...
		if err == nil {
			...
			if expectedPKIID != nil && !bytes.Equal(pkiID, expectedPKIID) {
				...
				c.logger.Debugf("Remote endpoint claims to be a different peer, expected", expectedPKIID, "but got", pkiID)
				cc.Close()
				cf()
				return nil, errors.New("Authentication failure")
			}
			priStream, err := cl.GossipPriStream(ctx)
			if err != nil {
				cc.Close()
				cf()
				return nil, errors.WithStack(err)
			}
			conn := newConnection(cl, cc, stream, priStream, nil, nil)
...
func (c *commImpl) SendPri(msg *proto.SignedGossipMessage, peers ...*RemotePeer) {
	if c.isStopping() || len(peers) == 0 {
		return
	}
	//c.logger.Debug("Entering, sending", msg, "to ", len(peers), "peers")
	...
	for _, peer := range peers {
		go func(peer *RemotePeer, msg *proto.SignedGossipMessage) {
			c.sendToEndpointPri(peer, msg, nonBlockingSend)
		}(peer, msg)
	}
}
...
func (c *commImpl) sendToEndpoint(peer *RemotePeer, msg *proto.SignedGossipMessage, shouldBlock blockingBehavior) {
	...
	if err == nil {
		disConnectOnErr := func(err error) {
			c.logger.Debugf("%v isn't responsive: %v", peer, err)
			c.disconnect(peer.PKIID)
		}
		conn.send(msg, disConnectOnErr, shouldBlock)
		return
	}
	c.logger.Debugf("Failed obtaining connection for %v reason: %v", peer, err)
	c.disconnect(peer.PKIID)
}

func (c *commImpl) sendToEndpointPri(peer *RemotePeer, msg *proto.SignedGossipMessage, shouldBlock blockingBehavior) {
	if c.isStopping() {
		return
	}
	//c.logger.Debug("Entering, Sending to", peer.Endpoint, ", msg:", msg)
	...
	if err == nil {
		...
		conn.sendPri(msg, disConnectOnErr, shouldBlock)
		return
	}
	c.logger.Debugf("Failed obtaining connection for %v reason: %v", peer, err)
	c.disconnect(peer.PKIID)
...
func (c *commImpl) Accept(acceptor common.MessageAcceptor) <-chan proto.ReceivedMessage {
	...
	if c.isStopping() {
		c.logger.Debugf("Accept() called but comm module is stopping, returning empty channel")
		return specificChan
	}
...
func (c *commImpl) authenticateRemotePeer(stream stream, initiator bool) (*proto.ConnectionInfo, error) {
	...
	if receivedMsg == nil {
		c.logger.Debugf("Expected connection message from", remoteAddress, "but got", receivedMsg)
...
func (c *commImpl) GossipPriStream(stream proto.Gossip_GossipPriStreamServer) error {
	if c.isStopping() {
		return fmt.Errorf("Shutting down")
	}
	...
	c.connStore.Lock()
	for pki, conn = range c.connStore.pki2Conn {
		if conn.info.Endpoint == extractRemoteAddress(stream) && conn.serverStream != nil {
			c.connStore.pki2Conn[pki].serverPriStream = stream
			break
		}
	}
	c.connStore.Unlock()
	...
	//c.logger.Debug("Servicing", extractRemoteAddress(stream))
	//
	//conn := c.connStore.onPriConnected(stream, connInfo)
	...
	conn.handler = interceptAcks(h, conn.info.ID, c.pubSub)

	defer func() {
		c.logger.Debug("Client", extractRemoteAddress(stream), " disconnected")
		c.connStore.closeByPKIid(conn.info.ID)
		conn.close()
	}()

	return conn.servicePriConnection()
}
