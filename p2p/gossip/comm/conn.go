...
func (cs *connectionStore) registerConn(connInfo *proto.ConnectionInfo, serverStream proto.Gossip_GossipStreamServer) *connection {
	conn := newConnection(nil, nil, nil, nil, serverStream, nil)
...
func newConnection(cl proto.GossipClient, c *grpc.ClientConn, cs proto.Gossip_GossipStreamClient, cps proto.Gossip_GossipPriStreamClient, ss proto.Gossip_GossipStreamServer, sps proto.Gossip_GossipPriStreamServer) *connection {
	connection := &connection{
		outBuff:         make(chan *msgSending, util.GetIntOrDefault("peer.gossip.sendBuffSize", defSendBuffSize)),
		outPriBuff:      make(chan *msgSending, util.GetIntOrDefault("peer.gossip.sendBuffSize", defSendBuffSize)),
		cl:              cl,
		conn:            c,
		clientStream:    cs,
		clienPriStream:  cps,
...
type connection struct {
	...
	clienPriStream  proto.Gossip_GossipPriStreamClient
	serverPriStream proto.Gossip_GossipPriStreamServer
...
func (conn *connection) close() {
	...
	if conn.clienPriStream != nil {
		conn.clienPriStream.CloseSend()
	}
	if conn.conn != nil {
		conn.conn.Close()
	}
...
func (conn *connection) sendPri(msg *proto.SignedGossipMessage, onErr func(error), shouldBlock blockingBehavior) {
	if conn.toDie() {
		conn.logger.Debug("Aborting sendPri() to ", conn.info.Endpoint, "because connection is closing")
		return
	}
	...
	if len(conn.outPriBuff) == cap(conn.outPriBuff) {
		if conn.logger.IsEnabledFor(logging.DEBUG) {
			conn.logger.Debug("Buffer to", conn.info.Endpoint, "overflowed, dropping message", msg.String())
		}
		if !shouldBlock {
...
func (conn *connection) servicePriConnection() error {
	errChan := make(chan error, 1)
	msgChan := make(chan *proto.SignedGossipMessage, util.GetIntOrDefault("peer.gossip.recvBuffSize", defRecvBuffSize))
	defer close(msgChan)

	go conn.readFromPriStream(errChan, msgChan)

	go conn.writeToPriStream()
...
func (conn *connection) writeToPriStream() {
	for !conn.toDie() {
		stream := conn.getPriStream()
		if stream == nil {
			conn.logger.Error(conn.pkiID, "Stream is nil, aborting!")
			return
		}
		select {
		case m := <-conn.outPriBuff:
			err := stream.Send(m.envelope)
			if err != nil {
				go m.onErr(err)
				return
...
func (conn *connection) drainOutputBuffer() {
	...
	// Drain the priority output buffer
	for len(conn.outPriBuff) > 0 {
		<-conn.outPriBuff
	}
}
...
func (conn *connection) readFromPriStream(errChan chan error, msgChan chan *proto.SignedGossipMessage) {
	defer func() {
		recover()
	}() // msgChan might be closed
	for !conn.toDie() {
...
func (conn *connection) getPriStream() stream {
	conn.Lock()
	defer conn.Unlock()

	if conn.clienPriStream != nil && conn.serverPriStream != nil {
		e := errors.New("Both client and server pri stream are not nil, something went wrong")
		conn.logger.Errorf("%+v", e)
	}

	if conn.clienPriStream != nil {
		return conn.clienPriStream
	}

	if conn.serverPriStream != nil {
		return conn.serverPriStream
	}
