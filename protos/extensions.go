...
func (mc *msgComparator) invalidationPolicy(this interface{}, that interface{}) common.InvalidationResult {
	...
	if thisMsg.IsTransactionMsg() && thatMsg.IsTransactionMsg() {
		return mc.txInvalidationPolicy(thisMsg.GetTransactionMsg(), thatMsg.GetTransactionMsg())
	}
...
func (mc *msgComparator) txInvalidationPolicy(thisTxMsg *TransactionMessage, thatTxMsg *TransactionMessage) common.InvalidationResult {
	if bytes.Equal(thisTxMsg.Transaction.SenderPubKey, thatTxMsg.Transaction.SenderPubKey) {
		if thisTxMsg.Transaction.Nonce == thatTxMsg.Transaction.Nonce {
			return common.MessageInvalidated
		}
	}
	return common.MessageNoAction
	...
	//}
	//
	//diff := abs(thisTxMsg.Transaction.Nonce, thatTxMsg.Transaction.Nonce)
	//if diff <= uint64(mc.dataBlockStorageSize) {
	//   return common.MessageNoAction
	//}
	//if thisTxMsg.Transaction.Nonce > thatTxMsg.Transaction.Nonce {
...
func (m *GossipMessage) IsTransactionMsg() bool {
	return m.GetTransactionMsg() != nil
}
...
// TxLedgerHeight returns the ledger height that is specified
// in the StateInfo message
func (msg *StateInfo) TxLedgerHeight() (uint64, error) {
	if msg.Properties != nil {
		return msg.Properties.TxLedgerHeight, nil
	}
