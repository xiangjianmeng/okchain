...
var (
	...
	EmptyString = []byte{0x80} // 0x80 = 128 对应string的起点，128 + 55 + 8
	EmptyList   = []byte{0xC0} // 0xC0 = 192 对应list的起点,192 + 55 + 8
)

// Encoder is implemented by types that require custom
// encoding rules or want to encode private fields.
// Encoder 是通过需要自定义编码规则或想要编码专用字段的类型实现的。
type Encoder interface {
	// EncodeRLP should write the RLP encoding of its receiver to w.
	// If the implementation is a pointer method, it may also be
	// called for nil pointers.
	// EncodeRLP应该将其接收器的RLP编码写入w。 如果这个实现是一个指针方法，那么它也可能被调用为零指针。
	//
...
// buffered.
// Encode将val的RLP编码写入w。 请注意，在某些情况下，Encode可能会执行很多小的写操作。 考虑让w缓冲。
//
...
// 首先获取了一个encbuf对象。 然后调用这个对象的encode方法
func Encode(w io.Writer, val interface{}) error {
	// 把w转换为 *encbuf 类型，ok表示转换成功还是失败，outer表示转换之后的io.Writer
	// 只要能转换，就进if，否则不进。对于io.Writer，仍可写各种类型
	if outer, ok := w.(*encbuf); ok {
		// Encode was called by some type's EncodeRLP.
		// Avoid copying by writing to the outer encbuf directly.
		return outer.encode(val)
	}
	// encbufPool相当于new一个encbuf，具体：&encbuf{sizebuf: make([]byte, 9)}
	eb := encbufPool.Get().(*encbuf)
	defer encbufPool.Put(eb)
	eb.reset() // 除sizebuf外都重置为0
	// 解构数据
...
/* 2018-05-06 郑超
* 重要的结构体
* str字段包含了所有的内容，除了列表的头部。
* lheads列表的头部记录【重要】
* lhsize字段记录了lheads的长度
* sizebuf是9个字节大小的辅助buffer，专门用来处理uint的编码的。
 */
type encbuf struct {
...
// size字段记录了包含列表头的编码后的数据的总长度。
type listhead struct {
	offset int // index of this header in string data 字符串数据中的这个header的索引
	size   int // total size of encoded data (including list headers) 总编码长度（包含list头）
}
...
// buf must be at least 9 bytes long. decode时使用
...
func (w *encbuf) encode(val interface{}) error {
	...
	// 2，设置w.str, w.lheads, w.lhsize；listhead的offset和size等
...
// 遍历结构体的每一个元素，找到每一个元素类型对应的writer方法
func makeStructWriter(typ reflect.Type) (writer, error) {
	...
	// 参数val是结构体的值
	writer := func(val reflect.Value, w *encbuf) error {
		// 生成一个{offset: 0, size: 0}的lheads
		lh := w.list()
		// 遍历 f 里就 index 和 info 两个字段。info就读写两个方法
...
func putint(b []byte, i uint64) (size int) {
	switch {
	case i < (1 << 8): // 左移n位就是乘以2的n次方。把"<<"左边的运算数的各二进位全部左移若干位
		b[0] = byte(i)
		return 1
	case i < (1 << 16):
		b[0] = byte(i >> 8) // 右移n位就是除以2的n次方
		b[1] = byte(i)
		return 2
	case i < (1 << 24):
		b[0] = byte(i >> 16)
