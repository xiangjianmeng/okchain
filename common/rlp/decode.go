...
func DecodeBytes(b []byte, val interface{}) error {
	...
	return nil
	//return proto.Unmarshal(b, val.(proto.Message))
}
...
// Stream可以用于输入流的（piecemeal零碎）解码。 这非常有用如果输入非常大或者某种类型的解码规则取决于输入结构。
// Stream不保留内部缓冲区。 在对一个值进行解码之后，输入读取器将被定位在下一个值的类型信息之前。
//
